...
public class AdminCommentController extends AbstractController{
	...
	public String articleList(@PathVariable(value="refId") int refId, Model model, HttpServletResponse response) {
		putAdmin(model, response);
		List<Comment> list = commtService.getCommentsByRef(refId);
	...
	public void postDel(@RequestParam("refId") int refId, @RequestParam("commtId") int commtId, Model model, HttpServletResponse response) {
		try {
			commtService.deleteComment(commtId);
			response.sendRedirect(ViewUtil.getContextPath() + "/admin/comment_list-" + refId);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
